...
public partial class CertificateInfoWindow : Window
{
    ...
    public CertificateInfoWindow(ConnectionSecurityInfo info, string pageUrl)
    {
        ...
        var (glyph, title, detail, color) = (info.SecurityState, isHttps, info.TrustedViaProxyCa) switch
        {
            ...
            ("secure", _, true) or (_, true, true) => ("🔒", "Secure · intercepted by the PII proxy",
                "This connection is encrypted to the proxy. The certificate was issued by the proxy CA that this browser trusts in-process only (not installed on Windows).", "#FF288879"),
        ...
        SummaryDetail.Visibility = string.IsNullOrEmpty(detail) ? Visibility.Collapsed : Visibility.Visible;
        SummaryBadge.Background = new SolidColorBrush((Color)ColorConverter.ConvertFromString(color));
    ...
    private void ShowCertificate(X509Certificate2? cert)
    {
        ...
        NoCertText.Visibility = has ? Visibility.Collapsed : Visibility.Visible;
        CertGrid.Visibility = has ? Visibility.Visible : Visibility.Collapsed;
    ...
    private void OnCopyPemClick(object sender, RoutedEventArgs e)
    {
        if ((ChainList.SelectedItem as ChainEntry)?.Certificate is { } cert)
        {
